ANP.pages = {
  ...
  dashboard() {
    ...
    const currentTopic = ANP.state.currentTopicId ? topics.find(t => t.ID === ANP.state.currentTopicId) : null;

    return `<div class="section-stack">
      <div class="grid cols-4">
        ${ANP.ui.metric('연구과제', topics.length, '등록된 전체 연구과제')}
      ...
      <div class="grid cols-2">
        <div class="card">
          <div class="card-head"><h2>현재 연구 범위</h2></div>
          <div class="list-card">
            <div class="td-title">${ANP.ui.esc(currentTopic?.Title || '전체 연구과제')}</div>
            <p class="page-desc">${ANP.ui.esc(currentTopic?.Description || '상단 연구과제 선택에서 특정 연구 범위를 선택할 수 있습니다.')}</p>
            <div>${currentTopic ? ANP.ui.badge(currentTopic.Status) : ANP.ui.badge('전체')}</div>
          </div>
        </div>
        <div class="card">
  ...
  recent(rows, title, body) {
    ...
    return rows.slice(0, 5).map(r => `<div class="list-card">
      <div class="td-title" title="${ANP.ui.esc(r[title])}">${ANP.ui.esc(r[title])}</div>
      <p class="page-desc" title="${ANP.ui.esc(r[body])}">${ANP.ui.esc(r[body])}</p>
      <div>${ANP.ui.badge(r.Status)}</div>
    </div>`).join('');
  },

  topics() {
    const rows = ANP.ui.applyFilter(ANP.state.data?.topics || [], 'topics', ['Title','Description','Status','OwnerName']);
    return `<div class="section-stack"><div class="card">
      ${ANP.ui.toolbar({ title:'연구과제 목록', desc:'여러 연구과제를 관리합니다.', filterKey:'topics', actionLabel:'연구과제 추가', actionType:'topic' })}
      ${ANP.ui.table(rows,[
        {key:'Title',label:'제목',type:'title',width:'24%'},
        {key:'Description',label:'설명',type:'pre',width:'36%'},
        {key:'OwnerName',label:'담당자',width:'14%'},
        {key:'Status',label:'상태',type:'status',width:'12%'},
        {key:'UpdatedAt',label:'수정일',width:'14%'}
      ])}
    </div></div>`;
  },
  ...
  roadmap() {
    ...
    return `<div class="section-stack"><div class="card">
      ${ANP.ui.toolbar({ title:'로드맵', desc:'진행 단계와 담당자를 관리합니다.', filterKey:'roadmap', actionLabel:'로드맵 추가', actionType:'roadmap' })}
      ${ANP.ui.table(rows,[
        {key:'TopicTitle',label:'연구과제',type:'title',width:'22%'},
        {key:'Title',label:'단계',type:'title',width:'20%'},
        {key:'Status',label:'상태',type:'status',width:'10%'},
        {key:'Progress',label:'진행률',type:'progress',width:'12%'},
        {key:'OwnerName',label:'담당자',width:'12%'},
        {key:'Note',label:'메모',type:'pre',width:'24%'}
      ])}
    </div></div>`;
  },
  ...
  opinion() {
    ...
    return `<div class="section-stack"><div class="card">
      ${ANP.ui.toolbar({ title:'의견', desc:'팀원 의견을 기록합니다.', filterKey:'opinion', actionLabel:'의견 등록', actionType:'opinion' })}
      ${ANP.ui.table(rows,[
        {key:'TopicTitle',label:'연구과제',type:'title',width:'20%'},
        {key:'Title',label:'제목',type:'title',width:'18%'},
        {key:'Opinion',label:'의견',type:'pre',width:'32%'},
        {key:'AuthorName',label:'작성자',width:'10%'},
        {key:'Status',label:'상태',type:'status',width:'8%'},
  ...
  decision() {
    ...
    return `<div class="section-stack"><div class="card">
      ${ANP.ui.toolbar({ title:'의사결정', desc:'작은 의사결정을 축적합니다.', filterKey:'decision', actionLabel:'의사결정 등록', actionType:'decision' })}
      ${ANP.ui.table(rows,[
        {key:'TopicTitle',label:'연구과제',type:'title',width:'18%'},
        {key:'Title',label:'제목',type:'title',width:'18%'},
        {key:'Decision',label:'결정',type:'pre',width:'24%'},
        {key:'Reason',label:'이유',type:'pre',width:'22%'},
        {key:'AuthorName',label:'작성자',width:'8%'},
        {key:'Status',label:'상태',type:'status',width:'10%'}
      ])}
    </div></div>`;
  },
  ...
  framework() {
    ...
    return `<div class="section-stack"><div class="card">
      ${ANP.ui.toolbar({ title:'프레임워크', desc:'합의된 구조를 관리합니다.', filterKey:'framework', actionLabel:'프레임워크 추가', actionType:'framework' })}
      ${ANP.ui.table(rows,[
        {key:'TopicTitle',label:'연구과제',type:'title',width:'18%'},
        {key:'Category',label:'분류',width:'12%'},
        {key:'Title',label:'제목',type:'title',width:'18%'},
        {key:'Content',label:'내용',type:'pre',width:'34%'},
        {key:'Status',label:'상태',type:'status',width:'8%'},
        {key:'Version',label:'버전',width:'10%'}
      ])}
    </div></div>`;
  },
  ...
  settings() {
    ...
    return `<div class="section-stack"><div class="card">
      ${ANP.ui.toolbar({ title:'설정', desc:'시스템 설정을 관리합니다.', filterKey:'settings' })}
      ${ANP.ui.table(rows,[
        {key:'Key',label:'설정키',type:'title',width:'20%'},
        {key:'Value',label:'값',type:'title',width:'28%'},
        {key:'Description',label:'설명',type:'pre',width:'34%'},
        {key:'UpdatedAt',label:'수정일',width:'18%'}
      ])}
    </div></div>`;
  },
  ...
  members() {
    ...
    return `<div class="section-stack"><div class="card">
      ${ANP.ui.toolbar({ title:'권한관리', desc:'사용자와 역할을 관리합니다. 사번(ID)은 내부키로 사용하며 목록에는 노출하지 않습니다.', filterKey:'members', actionLabel:'사용자 추가', actionType:'member' })}
      ${ANP.ui.table(rows,[
        {key:'LdapDisplay',label:'LdapID (이름)',type:'title',width:'22%'},
        {key:'Email',label:'이메일',width:'24%'},
        {key:'Department',label:'부서',width:'14%'},
        {key:'Team',label:'팀',width:'14%'},
        {key:'RoleLabel',label:'역할',width:'12%'},
        {key:'Status',label:'상태',type:'status',width:'14%'}
      ])}
    </div></div>`;
  },
  ...
  openForm(type) {
    ...
    root.querySelector('[data-dialog="confirm"]').onclick = async () => {
      ...
      try {
        await ANP.api.mutate(f.action, data);
        ANP.ui.closeModal();
