ANP.ui = {
  ...
  optionList(rows, valueKey, labelKey, selected = '', first = '선택하세요.', formatter) {
    ...
    (rows || []).filter(r => r.Enabled !== 'N' && r.Status !== '미사용').forEach(r => {
  ...
  topicOptions(selected='') { return this.optionList(ANP.state.data?.topics || [], 'ID', 'Title', selected, '선택하세요.'); },
  ...
  filterBox(key, placeholder='목록에서 검색하세요.') {
    ...
    return `<input class="input filter-input" data-filter="${this.esc(key)}" value="${this.esc(v)}" placeholder="${this.esc(placeholder)}" />`;
  },
  ...
  cell(row, col) {
    ...
    return `<div class="td-compact" title="${this.esc(value)}">${this.esc(value || '')}</div>`;
  },
  ...
  collect(ids) { const out = {}; ids.forEach(id => { const el = document.getElementById(id); out[id] = el ? el.value.trim() : ''; }); return out; },
  requireSession() { if (!ANP.state.session.userId || !ANP.state.session.writeKey) { this.toast('먼저 작성 정보를 설정해주세요.', 'warning'); ANP.app.openSession(); return false; } return true; }
};
